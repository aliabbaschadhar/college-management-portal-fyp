...
export function AlumniDashboardHome({
  ...
  const formattedGradDate = graduationDate
  ...
  return (
    ...
    >
      <PageHeader
        ...
        subtitle="Official Graduation Portal • Academic Degree Conferred"
        breadcrumbs={[
      ...
      {/* Real Live Database Attribute Summary Grid (Zero Dummy Data) */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <Card className="border border-border bg-card p-6 rounded-3xl shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Conferred Date</p>
              <h3 className="text-base font-extrabold text-foreground mt-0.5">{formattedGradDate}</h3>
            </div>
          </div>
        </Card>
      </div>
          ...
          <div className="py-4">
            {gradesheetUrl ? (
              <div className="space-y-4">
                <div className="flex items-center justify-between bg-muted/40 p-3 rounded-2xl border border-border text-xs font-semibold">
                  <span>Document Format: <strong>{isPdf ? "PDF Transcript" : "Image Gradesheet"}</strong></span>
                  <a
                    href={gradesheetUrl}
                    target="_blank"
                ...
                <div className="rounded-2xl border border-border overflow-hidden bg-black/5 dark:bg-black/40 min-h-[450px] flex items-center justify-center">
                  {isPdf ? (
                    <iframe
                      src={gradesheetUrl}
                      className="w-full h-[550px] border-none rounded-2xl"
                      title="Degree Gradesheet PDF"
                    />
                  ) : (
